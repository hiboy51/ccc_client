...
export default class Display extends cc.Component {
    ...
    protected _onNetworkDisconnected(prestate: NetStateEnum) {
        ...
        cc.log(`Disconnected Listener by Key ${this._sharedSignal ? this._realSwitch : switchKey} worked`);
    }
    ...
    onLoad() {
        // 监听断网
        this._savedDisconnectCb = SYEventManager.Instance.getEventTarget().on(CustomEventDefine.CE_NETWORK_CLOSED, this._onNetworkDisconnected, this);
        this._registerSwitchKey();
    }

    onDestroy() {
        this.unschedule(this._onTick);
        SYEventManager.Instance.getEventTarget().off(CustomEventDefine.CE_NETWORK_CLOSED, this._savedDisconnectCb, this);
